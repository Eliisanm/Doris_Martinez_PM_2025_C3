...
void lrctura(int vec[], int T);    /*prototipos de funciones. */
void imprime(int *X, int *Y, int *Z, int T);     /* Observa que en los parametros, para indicar que lo que se recibe es un arreglo, se puede escribir VEC[] o *VEC. */
...
void main (void)
{
 ...
 Lectura(VE1, MAX);
 /* Se llama a la funcion Lectura. Observa que el paso del arreglo de la funcion es por refrencia. solo se debe incluir el nombre del arreglo. */
  Lectura(VE2, MAX);
